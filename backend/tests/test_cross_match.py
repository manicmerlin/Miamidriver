...
def test_dior_l_finds_other_brands_within_tolerance():
    # Dior Monsieur L (Bohan, researched): chest 23.0, length 30.5, shoulder 19.5, sleeve 25.0
    src = [
        _meas("chest", 23.0),
        _meas("length", 30.5),
        _meas("shoulder", 19.5),
    ...
    matches = find_cross_matches(
        ...
        exclude=(
            ...
            "Monsieur Long-Sleeve Sport Shirt",
            "Marc Bohan Era (1970–1989)",
            "L",
        ),
    )
    assert matches, "expected at least one cross-match"
    # We expect SOMETHING from another brand to land within tolerance.
    other_brands = {m.entry.brand for m in matches if m.entry.brand != "Christian Dior Monsieur"}
    assert other_brands, f"no cross-brand matches: {matches}"


def test_excludes_source_garment_itself():
    src = [_meas("chest", 23.0), _meas("length", 30.5)]
    matches = find_cross_matches(
        source_measurements=src,
        source_garment_type=GarmentType.shirt,
        exclude=(
    ...
    for m in matches:
        assert not (
            ...
            and m.entry.line == "Monsieur Long-Sleeve Sport Shirt"
...
def test_ranks_closer_above_farther():
    src = [_meas("chest", 23.0), _meas("length", 30.5), _meas("shoulder", 19.5)]
    matches = find_cross_matches(
        source_measurements=src,
        source_garment_type=GarmentType.shirt,
        exclude=(
